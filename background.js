...
async function getMonitorStatus() {
  try {
    ...
    const twitchTabs = allTabs.filter(
      (tab) => tab.url && tab.url.includes("twitch.tv")
    );
...
async function executeScripts(tabId) {
  try {
    ...
    console.log(
      `Twitch Chat Overlay Cleaner executed successfully on tab: ${tabId}`
    );
  } catch (error) {
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (changeInfo.url && isTwitchPopoutChat(changeInfo.url)) {
    console.log(
      `[Service Worker] URL changed in tab ${tabId}, executing scripts...`
    );
  ...
  if (changeInfo.status === "loading" && isTwitchPopoutChat(tab.url)) {
    console.log(
      `[Service Worker] Page refresh detected in tab ${tabId}, waiting for complete...`
    );
    ...
    const checkComplete = async () => {
      try {
        ...
        if (updatedTab.status === "complete") {
          console.log(
            `[Service Worker] Tab ${tabId} loaded completely after refresh, executing scripts...`
          );
          await executeScripts(tabId);
        } else {
        ...
        console.error(
          `[Service Worker] Error checking tab ${tabId} status:`,
          error
        );
      }
    };
